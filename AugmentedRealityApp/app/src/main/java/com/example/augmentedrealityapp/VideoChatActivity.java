...
import com.example.augmentedrealityapp.capturers.AndroidCameraCapturer;
import com.example.augmentedrealityapp.observers.CustomPeerConnectionObserver;
import com.example.augmentedrealityapp.observers.CustomSdpObserver;
import com.example.augmentedrealityapp.services.IceServerService;
import com.example.augmentedrealityapp.services.SignalrService;
...
public class VideoChatActivity extends AppCompatActivity {
    ...
    private SignalrService mSignalingService;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSignalingService = new SignalrService( "/signaling");
        initViews();
    }
    ...
    protected void onDestroy() {
        ...
        new Handler().postDelayed(() -> mSignalingService.dispose(), 1000);
        super.onDestroy();
    }
    ...
    private void start() {
        ...
        mSignalingService.connect(() -> {
            if (mSignalingService.isConnected()) {
                mSignalingService.invoke("CreateOrJoinRoom", mRoomName);
            }
        });
    ...
    private void defineSignaling() {
        mSignalingService.define("log", log -> {
            Log.i(TAG, "log [" + log + "]");
        }, String.class);

        mSignalingService.define("created", () -> {
            mIsInitiator = true;
        });

        mSignalingService.define("joined", () -> {
            mIsChannelReady = true;
        });

        mSignalingService.define("message", message -> {
    ...
    private void sendMessage(Object message) {
        ...
        if(mSignalingService != null) {
            mSignalingService.invoke("SendMessage", message, mRoomName);
        }
    }
    ...
    private void hangup() {
        ...
        stopPeerConnection();
        sendMessage("bye");
        mSignalingService.invoke("LeaveRoom", mRoomName);
    }
